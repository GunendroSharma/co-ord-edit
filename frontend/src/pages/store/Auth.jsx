...
export function Signup() {
  ...
  useEffect(() => {
    if (refCode) {
      api.post("/referrals/validate", { code: refCode }).then((r) => { if (r.data.valid) setRefInfo(r.data); });
    }
  }, [refCode]);
  ...
  const submit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await api.post("/auth/signup", { ...form, referred_by: refCode || undefined });
      ...
      window.location.href = data.user.welcome_coupon ? `/account?welcome=${data.user.welcome_coupon}` : "/account";
    }
  ...
  return (
    <div>
      ...
      <div className="max-w-md mx-auto px-6 py-16">
        ...
        {refInfo && (
          <div className="mb-6 p-4 bg-[color:var(--sf-secondary)] border border-[color:var(--sf-primary)] text-sm" data-testid="referral-banner">
            <div className="flex items-center gap-2 mb-1"><Gift size={16} /><b>You've been invited by {refInfo.referrer_name}</b></div>
            <p className="text-[color:var(--sf-text-soft)]">₹300 off your first order over ₹999 — applied automatically after signup.</p>
          </div>
        )}
...
export function Account() {
  ...
  useEffect(() => {
    if (user) {
      ...
      api.get("/referrals/me").then((r) => setReferral(r.data));
      if (welcome) toast.success(`Welcome! Use ${welcome} at checkout for ₹300 off.`, { duration: 8000 });
    }
  }, [user, welcome]);
  if (!user) { nav("/login"); return null; }
  const shareUrl = `${window.location.origin}/signup?ref=${referral?.code || ""}`;
  const copyLink = () => { navigator.clipboard.writeText(shareUrl); toast.success("Referral link copied"); };
  ...
  return (
    <div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-16" data-testid="account-page">
        ...
        {/* Referral card */}
        {referral && (
          <div className="mt-10 border p-8 bg-white relative overflow-hidden" data-testid="referral-card">
            <div className="absolute -right-8 -top-8 w-40 h-40 rounded-full bg-[color:var(--sf-primary)] opacity-20"></div>
            <div className="overline text-[color:var(--sf-text-soft)] mb-2">Give ₹300, Get ₹300</div>
            <h2 className="font-serif-display text-3xl mb-3">Your referral link</h2>
            <p className="text-sm text-[color:var(--sf-text-soft)] max-w-lg mb-6">{referral.message}</p>
            <div className="flex gap-2 flex-wrap">
              <div className="flex-1 min-w-[260px] flex border">
                <input readOnly value={shareUrl} className="flex-1 px-3 py-2 text-sm bg-transparent" data-testid="referral-link-input" />
                <button onClick={copyLink} className="px-4 border-l text-sm flex items-center gap-1" data-testid="referral-copy"><Copy size={14} /> Copy</button>
              </div>
              <div className="flex gap-6 items-center px-4 border py-2">
                <div><div className="text-xs text-[color:var(--sf-text-soft)]">Code</div><div className="font-mono" data-testid="referral-code">{referral.code}</div></div>
                <div><div className="text-xs text-[color:var(--sf-text-soft)]">Invites</div><div data-testid="referral-invites">{referral.invites}</div></div>
                <div><div className="text-xs text-[color:var(--sf-text-soft)]">Credits</div><div data-testid="referral-credits">₹{referral.credits_earned}</div></div>
              </div>
            </div>
          </div>
        )}
